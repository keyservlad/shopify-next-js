...
// TODO create handle when an error occurs
...
export async function createCheckoutGiftCard(lineItems, email) {
  const lineItemsObject = lineItems.map((item) => {
    return `{
        variantId: "${item.id}",
        quantity: ${item.variantQuantity}
  ...
  const query = gql`
      mutation {
        checkoutCreate(input: { lineItems: [${lineItemsObject}], email: "${email}"}) {
          checkout {
            id
            webUrl
            lineItems(first: 25) {
