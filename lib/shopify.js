        ...
        products(first: 250) {
          edges {
            node {
              ...
              prix_membre: variantBySelectedOptions(
                selectedOptions: { name: "membre", value: "Prix membre" }
              ) {
                priceV2 {
                  amount
                }
...
export async function getSerieLimitee() {
  const query = gql`
    query {
      collection(handle: "serie-limite") {
        handle
        products(first: 1) {
          nodes {
            id
  ...
  const response = await Shopify(query);
  const products = response.collection.products.nodes[0]
    ? response.collection.products.nodes[0]
    : [];
  return products;
}

export async function getAllProductsCoffretsCadeaux() {
  const query = gql`
    query {
      collection(handle: "coffrets-cadeaux") {
        products(first: 9) {
          edges {
            node {
              id
...
export async function getAllProductsGiftCards() {
  const query = gql`
    query {
      collection(handle: "carte-cadeau") {
        products(first: 1) {
          edges {
            node {
              id
  ...
  const query = gql`
    {
      productByHandle(handle: "${handle}") {
        ...
        accord_mets_vins: metafield(namespace: "my_fields", key: "accord_mets_vins") {
          value
          type
        }
        service: metafield(namespace: "my_fields", key: "service") {
          value
          type
        }
        a_l_oeil: metafield(namespace: "my_fields", key: "a_l_oeil") {
          value
          type
        }
        au_nez: metafield(namespace: "my_fields", key: "au_nez") {
          value
          type
        }
        en_bouche_leger_puissant: metafield(namespace: "my_fields", key: "en_bouche_leger_puissant") {
          value
          type
        }
        en_bouche_sec_doux: metafield(namespace: "my_fields", key: "en_bouche_sec_doux") {
          value
          type
        }
        en_bouche_souple_tannique: metafield(namespace: "my_fields", key: "en_bouche_souple_tannique") {
          value
          type
        }
        variants(first: 2) {
...
export async function getAllCards() {
  ...
  const response = await Shopify(query);
  const cartes = response.collection.products
