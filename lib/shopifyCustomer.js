...
export async function resetPasswordByUrl(url, password) {
  ...
  const response = await Shopify(query);
  const customerAccessToken = response.customerResetByUrl
    ? response.customerResetByUrl
    : [];
  return customerAccessToken;
}

export async function getCustomerLight(clientAccessToken) {
  const query = gql`
    query{
  ...
  const query = gql`
    query{
      customer(customerAccessToken: "${clientAccessToken}") {
        ...
        plateforme: metafield(namespace: "custom", key: "plateforme") {
          id
          key
          namespace
          value
        }
        nextExpirationdate: metafield(namespace: "custom", key: "nextExpirationdate") {
          id
          key
          namespace
          value
        }
        nextPlateforme: metafield(namespace: "custom", key: "nextPlateforme") {
          id
          key
          namespace
          value
        }
        nextCarte: metafield(namespace: "custom", key: "nextCarte") {
          id
          key
          namespace
          value
        }
        nextIsDomicile: metafield(namespace: "custom", key: "nextIsDomicile") {
          id
          key
          namespace
          value
