    ...
    query {
      customers(query: "${email}", first: 1) {
        nodes {
          ...
          isPlusPlus: metafield(namespace: "custom", key: "isPlusPlus") {
            id
            key
            namespace
            value
...
export async function deleteMetafield(idMetafield) {
  // const inputt = input.replace(/"([^"]+)":/g, "$1:");
  const query = gql`
    mutation {
  ...
  const response = await Shopify(query);
  const customer = response.metafieldDelete ? response.metafieldDelete : [];
  return customer;
}
