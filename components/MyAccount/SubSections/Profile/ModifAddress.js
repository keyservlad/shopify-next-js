...
const schema = object({
  firstName: string().required("Veuillez entrer votre prénoma"),
  lastName: string().required("Veuillez entrer votre noma"),
  address: string().required("Veuillez entrer votre adressea"),
  country: string().required("Veuillez entrer votre paysa"),
  city: string().required("Veuillez entrer votre Villea"),
  zipCode: string().required("Veuillez entrer votre code postala"),
  phone: string()
    .required("Veuillez entrer votre numéro de téléphonea")
    .matches(phoneRegExp, "Numéro de léléphone non validea")
    .min(10, "Trop court"),
});
...
const ModifAddress = ({ setIsAddressEditing, address }) => {
  ...
  return (
    <div className="lg:col-span-9 p-4 sm:p-6">
      <button
        className="hover:opacity-70 cursor-pointer flex flex-row items-center mb-4 text-sm"
        onClick={() => {
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <fieldset>
          ...
          <p className="text-sm text-gray-500">
            Sélectionnez l&#39;adresse à laquelle vous souhaitez recevoir votre
            vin
          </p>
          <DeliveryAdress
            control={control}
