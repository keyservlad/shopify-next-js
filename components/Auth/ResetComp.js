...
const ResetComp = ({ activation_url }) => {
  ...
  async function onSubmit(values) {
    ...
    // modif password
    const modifUser = await resetPasswordByUrl(activation_url, values.password);
    if (modifUser.customerUserErrors.length !== 0) {
      setIsLoading(false);
      setError("password2", {
        type: "custom",
      ...
      router.push({
        pathname: "/login",
        query: { success: "mot de passe modifie" },
      });
    }
